...
export interface Incident {
  ...
  title: string;
  timestamp: Date;
  provider: string; // opened up from "aws" | "azure" | "openai"
}
...
const IncidentCard = ({ incident, onClick }: IncidentCardProps) => {
  const StatusIcon = statusConfig[incident.status]?.icon ?? Activity;
  const statusColor = statusConfig[incident.status]?.color ?? "text-muted-foreground";
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        <span
          className={`text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full font-medium ${
            severityConfig[incident.severity]?.class ?? "severity-low"
          }`}
        >
